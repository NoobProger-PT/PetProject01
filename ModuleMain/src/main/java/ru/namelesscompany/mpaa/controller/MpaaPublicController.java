...
@RequestMapping(path = "/public/mpaa")
@Slf4j
@Validated
public class MpaaPublicController {

    private final PublicMpaaService service;

    @GetMapping("/{mpaaId}")
    public MpaaDto get(@PathVariable @Positive long mpaaId) {
        log.info("Получение гостем рейтинга Mpaa с id: {}", mpaaId);
        return service.getById(mpaaId);
    }

    @GetMapping("/all")
    public List<MpaaDto> getAll() {
        log.info("Получение гостем всех райтингов Mpaa");
